.ONESHELL:
.PHONY: all beautiful black build clean commit continue isort jacobus pypi test toml_sorted_pyproject upload version works
...
continue:
	git rebase --empty=drop --continue
...
test:
	...
	conda run -n test311 python -c 'import keyalias.tests; keyalias.tests.test()';
	conda run -n test311 pip install mypy;
	conda run -n test311 python -m mypy -p keyalias;
